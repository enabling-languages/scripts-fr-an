...
function startup(data, reason) {
 if (reason == 3 || reason == 5 || reason == 7) {     
  ...
  checkFile.append("Lohit-Bengali.ttf");
  if (!checkFile.exists()) {
   if (fromDir == null) linkFromDir();
   
   fromFile = fromDir.clone();
   fromFile.append("Lohit-Bengali.ttf");
   fromFile.copyTo(profiledir,"Lohit-Bengali.ttf");
   fontAdded = true;
  }
  ...
  if (!checkFile.exists()) { 
   ...
   fromFile = fromDir.clone();
   fromFile.append("Lohit-Bengali.ttf");
   fromFile.copyTo(fontdir,"Lohit-Bengali.ttf");
   fontAdded = true;
  } 
