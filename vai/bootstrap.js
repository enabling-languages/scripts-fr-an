...
function startup(data, reason) {
 if (reason == 3 || reason == 5 || reason == 7) {     
  ...
  function linkFromDir() {
   ...
   fromDir.append("vaifonts@openroad.net.au");
   fromDir.append("defaults");
  }
  
  //when v19 moves to beta, remove font check in root user profile dir
  checkFile = profiledir.clone();
  checkFile.append("Dukor.ttf");
  if (!checkFile.exists()) {
   if (fromDir == null) linkFromDir();
   
   fromFile = fromDir.clone();
   fromFile.append("Dukor.ttf");
   fromFile.copyTo(profiledir,"Dukor.ttf");

   fromFile = fromDir.clone();
   fromFile.append("DukorItalic.ttf");
   fromFile.copyTo(profiledir,"DukorItalic.ttf");
  ...
  if (!checkFile.exists()) { 
   ...
   fromFile = fromDir.clone();
   fromFile.append("Dukor.ttf");
   fromFile.copyTo(fontdir,"Dukor.ttf");
   
   fromFile = fromDir.clone();
   fromFile.append("DukorItalic.ttf");
   fromFile.copyTo(fontdir,"DukorItalic.ttf");
...
function shutdown(data, reason) {
 if (reason == 4) {
  ...
  checkFile = profiledir.clone();
  checkFile.append("DukorItalic.ttf");
  if (checkFile.exists()) checkFile.remove(true);
