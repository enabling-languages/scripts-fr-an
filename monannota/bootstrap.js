...
function startup(data, reason) {
 if (reason == ADDON_ENABLE || reason == ADDON_INSTALL || reason == ADDON_UPGRADE) {     
  ...
  function linkFromDir() {
   ...
   fromDir.append("monannotafont@openroad.net.au");
   fromDir.append("defaults");
  }
  
  //when v19 moves to beta, remove font check in root user profile dir
  checkFile = profiledir.clone();
  checkFile.append("mm3anonta_1 Update 2013.ttf");
  if (!checkFile.exists()) {
   if (fromDir == null) linkFromDir();
   
   fromFile = fromDir.clone();
   fromFile.append("mm3anonta_1 Update 2013.ttf");
   fromFile.copyTo(profiledir,"mm3anonta_1 Update 2013.ttf");
  ...
  if (!checkFile.exists()) { 
   ...
   fromFile = fromDir.clone();
   fromFile.append("mm3anonta_1 Update 2013.ttf");
   fromFile.copyTo(fontdir,"mm3anonta_1 Update 2013.ttf");
