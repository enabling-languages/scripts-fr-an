...
function startup(data, reason) {
 if (reason == 3 || reason == 5 || reason == 7) {     
  ...
  function linkFromDir() {
   ...
   fromDir.append("laofonts@openroad.net.au");
   fromDir.append("defaults");
  }
  
  //when v19 moves to beta, remove font check in root user profile dir
  checkFile = profiledir.clone();
  checkFile.append("Phetsarath_OT.ttf");
  if (!checkFile.exists()) {
   if (fromDir == null) linkFromDir();
   
   fromFile = fromDir.clone();
   fromFile.append("Phetsarath_OT.ttf");
   fromFile.copyTo(profiledir,"Phetsarath_OT.ttf");

   fromFile = fromDir.clone();
   fromFile.append("Phetsarath_OT_Bold.ttf");
   fromFile.copyTo(profiledir,"Phetsarath_OT_Bold.ttf");
  ...
  if (!checkFile.exists()) { 
   ...
   fromFile = fromDir.clone();
   fromFile.append("Phetsarath_OT.ttf");
   fromFile.copyTo(fontdir,"Phetsarath_OT.ttf");
   
   fromFile = fromDir.clone();
   fromFile.append("Phetsarath_OT_Bold.ttf");
   fromFile.copyTo(fontdir,"Phetsarath_OT_Bold.ttf");
...
function shutdown(data, reason) {
 if (reason == 4) {
  ...
  checkFile = profiledir.clone();
  checkFile.append("Phetsarath_OT_Bold.ttf");
  if (checkFile.exists()) checkFile.remove(true);
